...
static const struct gpio_dt_spec led = GPIO_DT_SPEC_GET(DT_PATH(leds, red_led), gpios);
...
static int init_dfu_ramdisk_data(struct dfu_ramdisk_data *const data)
{
	...
	LOG_INF("disk %s sector count %u sector size %u", data->name, data->sector_count, data->sector_size);
...
static void msg_cb(struct usbd_context *const usbd_ctx, const struct usbd_msg *const msg)
{
	...
	if (msg->type == USBD_MSG_DFU_DOWNLOAD_COMPLETED) {
		if (IS_ENABLED(CONFIG_BOOTLOADER_MCUBOOT) && IS_ENABLED(CONFIG_APP_USB_DFU_USE_FLASH_BACKEND)) {
